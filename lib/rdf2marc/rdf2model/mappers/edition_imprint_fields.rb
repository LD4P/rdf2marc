    ...
    module Mappers
      class EditionImprintFields < BaseMapper
          ...
          pub_dists = pub_dist_terms.map do |pub_dist_term|
            {
                ...
                publication_distribution_dates: item.instance.query.path_all_literal([BF.date], subject_term: pub_dist_term)
            }
          end
        ...
        def publication_distributions_places(subject_term)
          item.instance.query.path_all_uri([BF.place], subject_term: subject_term).map { |place_uri| Resolver.resolve_label(place_uri) }
        end

        def publication_distributions_names(subject_term)
          item.instance.query.path_all_uri([[BF.agent, BF.Agent], BF.Agent], subject_term: subject_term).map do |agent_uri|
